...
def GetToken(app_key, secret_key):
    ...
    response = urlopen("https://openapi.baidu.com/oauth/2.0/token", data=bytes(urlencode(data),'utf8'))
...
class TTS(object):
    ...
    def say(self, text, spd=5, pit=5, vol=5, per=0):
        ...
        self.request = Request(url, data=bytes(urlencode(data),'utf8'))
        ...
        '''
        #print(response.read())
        #os.system(response.read())
        self.play_mp3(response.read())
        #mp3 = mp3play.load(response.read())
