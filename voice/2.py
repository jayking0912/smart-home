...
DEFAULT_APP_KEY = "o1A14nQdfrgKuRMMXGfDhlZy"
DEFAULT_SECRET_KEY = "cead88eb6c402ae2afc076b060839086"

def GetToken(app_key, secret_key):
    data = 'grant_type=client_credentials&client_id={0}&client_secret={1}'.format(app_key,secret_key)
    data=bytes(data,'utf8')
    response = urllib.request.Request("https://openapi.baidu.com/oauth/2.0/token", data=data)
    response_text = urllib.request.urlopen(response)
    response_result=response_text.read()
    response_result=response_result.decode()
...
class TTS(object):
    ...
    def say(self, text, spd=5, pit=5, vol=5, per=0):
        ...
        data = 'tex={0}&lan={1}&tok={2}&ctp=1&cuid=93489083242&spd={3}&pit={4}&vol={5}&per={6}'.format(text,self.language,self.token,spd,pit,vol,per)
        data=bytes(data,'utf8')
        self.request = urllib.request.Request(url, data =data)
...
class Recognizer(AudioSource):
    ...
    def get_token(self):
        data = 'grant_type=client_credentials&client_id={0}&client_secret={1}'.format(self.app_id,self.secret_id)
        data=bytes(data,'utf8')
        response = urllib.request.Request("https://openapi.baidu.com/oauth/2.0/token", data=data)
        response_text = urllib.request.urlopen(response)
        response_result=response_text.read()
